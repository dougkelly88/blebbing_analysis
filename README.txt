...
*Curvature length parameter (pix): the separation along the membrane between the point at which curvature is to
 be calculated and the two points on either side from which curvature is calculated using the three-point 
 method and SSS theorem (see http://mathworld.wolfram.com/SSSTheorem.html) - defaults to 10. 
*Threshold method: the (in built ImageJ) thresholding method used to separate membrane from background in the 
 intensity (actin) channel - defaults to "Moments". 
